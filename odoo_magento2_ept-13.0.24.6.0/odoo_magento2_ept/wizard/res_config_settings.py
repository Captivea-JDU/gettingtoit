...
class ResConfigSettings(models.TransientModel):
    ...
    magento_apply_tax_in_order = fields.Selection(
        ...
        copy=False, default="odoo_tax", help=""" For Magento Orders :- \n
    ...
    magento_analytic_tag_ids = fields.Many2many('account.analytic.tag','magento_res_config_analytic_account_tag_rel', string='Magento Analytic Tags')
    m_website_analytic_account_id = fields.Many2one('account.analytic.account',
                                                    string='Magento Website Analytic Account')
    m_website_analytic_tag_ids = fields.Many2many('account.analytic.tag','magento_website_res_config_analytic_account_tag_rel',
    ...
    def onchange_magento_instance_id(self):
        ...
        if magento_instance_id:
            ...
            self.magento_apply_tax_in_order = magento_instance_id.magento_apply_tax_in_order
            self.magento_invoice_tax_account_id = magento_instance_id.magento_invoice_tax_account_id
            self.magento_credit_tax_account_id = magento_instance_id.magento_credit_tax_account_id
            self.magento_analytic_account_id = magento_instance_id.magento_analytic_account_id.id,
            self.magento_analytic_tag_ids = [(6, 0,magento_instance_id.magento_analytic_tag_ids.ids)] if magento_instance_id.magento_analytic_tag_ids else False
    ...
    def execute(self):
        ...
        if self.magento_website_id:
            self.magento_website_id.write({
                ...
                'm_website_analytic_tag_ids': [(6, 0, self.m_website_analytic_tag_ids.ids)] if self.m_website_analytic_tag_ids else False,
            })
    ...
    def write_instance_vals(self, magento_instance_id):
        ...
        values.update({
            ...
            'magento_analytic_tag_ids': [(6, 0, self.magento_analytic_tag_ids.ids)] if self.magento_analytic_tag_ids else False,
        })
